...
const memories = [
  {
    id: 1,
    rotation: "3deg",
    caption: "something pretty for you 🌷",
    position: "left-2 top-2",
  },
  {
    id: 2,
    rotation: "-4deg",
    caption: "surrounded by flowers ♡",
    position: "right-2 top-[300px]",
  },
  {
    id: 3,
    rotation: "2deg",
    caption: "a little flower for a special day",
    position: "left-6 top-[600px]",
  },
];

export default function FloristMemories() {
  ...
  useEffect(() => {
    const loadPhotos = async () => {
      const { data, error } = await supabase
        ...
        .eq("memory_key", "florist");

      if (error) {
        console.error("Failed to load Florist memories:", error);
        return;
      }
  ...
  const handleUpload = async (event, id) => {
    ...
    try {
      ...
      const filePath = `florist/photo-${id}-${Date.now()}.${fileExtension}`;
      ...
      <div className="mx-auto max-w-lg">
        ...
        >
          ...
          <p className="mt-3 text-sm leading-6 text-gray-500">
            A little moment surrounded by pretty things.
          </p>
        </motion.div>
